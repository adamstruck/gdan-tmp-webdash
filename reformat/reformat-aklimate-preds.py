...
def qc_prediction(PREDICTION_C):
    ...
    tumor, subtype = re.split(r"_", PREDICTION_C)
    if tumor not in subtype:
        PREDICTION_C = re.sub(subtype, tumor+"_"+subtype, PREDICTION_C)
        one = PREDICTION_C.split('_')[0]
        two = PREDICTION_C.split('_')[2]
        PREDICTION_C = one +":"+one+"_"+two
        #print(PREDICTION_C)
    return PREDICTION_C
...
#####
# Read in file
raw_pred = pd.read_csv(pred_file, skiprows=19, sep='\t')
#raw_pred
